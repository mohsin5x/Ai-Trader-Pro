...
def _price_fmt(symbol: str, price: float) -> str:
    s = symbol.upper()
    if any(c in s for c in ("BTC","ETH")): return f"{price:,.0f}"
    if s in ("BNB","SOL","AVAX","DOT","LINK","ATOM","UNI","LTC"): return f"{price:,.2f}"
    if s in ("XRP","ADA","DOGE","MATIC"): return f"{price:,.4f}"
    if "XAU" in s or "XAG" in s: return f"{price:,.2f}"
    if s in ("US30","NAS100","SPX500"): return f"{price:,.1f}"
    return f"{price:,.4f}"
...
class WatchlistPage(ctk.CTkFrame):
    def __init__(self, parent, on_asset_click=None):
        ...
        for col, weight, text in [(0,3,"SYMBOL"),(1,5,"NAME"),(2,4,"PRICE"),(3,3,"CHANGE %")]:
            ...
            ctk.CTkLabel(hdr_row, text=text, font=SF.TINY(),
